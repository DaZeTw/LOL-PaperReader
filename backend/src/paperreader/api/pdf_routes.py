    ...
    try:
        for index, pdf_path in enumerate(saved_paths, start=1):
            ...
            try:
                ...
                asset_prefix = (
                    f"documents/{asset_identifier}/"
                    if document_id
                    else f"uploads/{asset_identifier}/"
                )
...
async def _run_pdf_job(
    saved_paths: List[Path], document_map: Optional[Dict[str, str]], job_id: str
) -> None:
    try:
...
async def qa_status():
    ...
    try:
        ...
        # Add debug logging
        print(
            f"[STATUS] Pipeline status check: building={status.get('building')}, ready={status.get('ready')}, has_cache={status.get('has_cache')}, chunks={status.get('chunks')}"
        )
        return status
...
async def parse_uploads_folder():
    ...
    for pdf_path in pdf_paths:
        ...
        if _PARSE_CANCEL_FLAG.is_set():
            print(
                f"[PDF] ⚠️ Parse operation cancelled - skipping remaining PDFs (stopped at {pdf_path.name})"
            )
            break
        ...
        try:
            ...
            if _PARSE_CANCEL_FLAG.is_set():
                print(
                    f"[PDF] ⚠️ Parse operation cancelled during parsing {pdf_path.name}: {e}"
                )
                break
            raise
        ...
        print(
            f"[PDF] Parser outputs: md={result.get('markdown_embedded')}, pages={len(result.get('page_images') or [])}, figures={len(result.get('figures') or [])}"
        )

        # Check cancel flag immediately after parsing
        if _PARSE_CANCEL_FLAG.is_set():
            print(
                f"[PDF] ⚠️ Parse operation cancelled after parsing {pdf_path.name} - stopping"
            )
            break
        ...
        if markdown_content:
            print(
                f"[PDF] Markdown content in result: Yes (length: {len(markdown_content)} chars)"
            )
        else:
        ...
        if not markdown_content:
            ...
            md_path = Path(
                result.get("markdown_embedded")
                or result.get("markdown_referenced")
                or ""
            )
            if md_path and md_path.exists():
                ...
                print(
                    f"[PDF] ⚠️ Parser didn't produce markdown content for {pdf_path.name}, skipping..."
                )
                continue
        ...
        if _PARSE_CANCEL_FLAG.is_set():
            print(
                f"[PDF] ⚠️ Operation cancelled before converting markdown for {pdf_path.name}"
            )
            break
        ...
        print(
            f"[PDF] 🚀 Chunking directly from parsed markdown (no file I/O) for {doc_id}..."
        )
        chunks = split_markdown_into_chunks(markdown_content, doc_id, str(pdf_path))
        if chunks:
            all_chunks.append(chunks)
            print(
                f"[PDF] ✅ Created {len(chunks)} chunks directly from parsed markdown"
            )
        else:
        ...
        if _PARSE_CANCEL_FLAG.is_set():
            print(
                f"[PDF] ⚠️ Operation cancelled before copying images for {pdf_path.name}"
            )
            break
        ...
        if figures_count > 0:
            ...
            for idx, p in enumerate(result.get("figures", []), 1):
                ...
                if _PARSE_CANCEL_FLAG.is_set():
                    print(
                        f"[PDF] ⚠️ Image copying cancelled - stopping at figures {idx}/{figures_count}"
                    )
                    break
                ...
                if pth.exists():
                    ...
                    try:
                        ...
                        if _PARSE_CANCEL_FLAG.is_set():
                            print(
                                f"[PDF] ⚠️ Image copying cancelled after copying figure {idx}/{figures_count}"
                            )
                            break
        ...
        if _PARSE_CANCEL_FLAG.is_set():
            print(
                f"[PDF] ⚠️ Operation cancelled before adding {pdf_path.name} to results"
            )
            break
    ...
    try:
        print(
            "[PDF] Building pipeline end-to-end (chunks + embeddings + store) with in-memory chunks..."
        )
        print(
            f"[PDF] 🔨 Calling rebuild_pipeline with {len(flat_chunks)} chunks directly (no file loading)..."
        )
        build_start = time.time()
        pipeline = await rebuild_pipeline(cfg, lazy_store=False, chunks=flat_chunks)
        build_elapsed = time.time() - build_start
        print(
            f"[PDF] ✅ Full pipeline ready with {len(pipeline.artifacts.chunks)} chunks; store built={pipeline._store_built}; build_time={build_elapsed:.2f}s"
        )
    except RuntimeError as e:
        if "cancelled" in str(e).lower():
            print(
                f"[PDF] ⚠️ Pipeline build was cancelled (output cleared during build): {e}"
            )
        else:
