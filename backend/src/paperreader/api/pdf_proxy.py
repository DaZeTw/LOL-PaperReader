...
router = APIRouter(prefix="/api/pdf", tags=["pdf-proxy"])
...
async def get_pdf_url_from_semantic_scholar(identifier: str, id_type: str = "doi") -> str | None:
    """
...
async def proxy_pdf(url: str, title: str = None):
    ...
    try:
        async with httpx.AsyncClient(follow_redirects=True, timeout=60.0) as client:
            ...
            print(f"[PDF Proxy] Successfully fetched PDF: {filename} ({len(response.content)} bytes)")
