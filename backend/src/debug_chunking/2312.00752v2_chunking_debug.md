...
![p3_img001](images/2312.00752v2-p3-img001.png)
...
We specifically choose 𝑠 𝐵 ( 𝑥 ) = Linear 𝑁 ( 𝑥 ) , 𝑠 𝐶 ( 𝑥 ) = Linear 𝑁 ( 𝑥 ) , 𝑠 Δ ( 𝑥 ) = Broadcast 𝐷 ( Linear 1 ( 𝑥 )) , and 𝜏 Δ = softplus , where Linear 𝑑 is a parameterized projection to dimension 𝑑 . The choice of 𝑠 Δ and 𝜏 Δ is due to a connection to RNN gating mechanisms explained in Section 3.5 .

5

# Page 6

![p6_img001](images/2312.00752v2-p6-img001.png)
...
1: 𝑨 : ( D , N ) ← Parameter ⊲ Represents structured 𝑁 × 𝑁 matrix 2: 𝑩 : ( D , N ) ← Parameter 3: 𝑪 : ( D , N ) ← Parameter 4: Δ : ( D ) ← 𝜏 Δ ( Parameter ) 5: 𝑨 , 𝑩 : ( D , N ) ← discretize ( Δ , 𝑨 , 𝑩 ) 6: 𝑦 ← SSM ( 𝑨 , 𝑩 , 𝑪 )( 𝑥 ) ⊲ Time-invariant: recurrence or convolution 7: return 𝑦

1: 𝑨 : ( D , N ) ← Parameter ⊲ Represents structured 𝑁 × 𝑁 matrix 2: 𝑩 : ( B , L , N ) ← 𝑠 𝐵 ( 𝑥 ) 3: 𝑪 : ( B , L , N ) ← 𝑠 𝐶 ( 𝑥 ) 4: Δ : ( B , L , D ) ← 𝜏 Δ ( Parameter + 𝑠 Δ ( 𝑥 ) ) 5: 𝑨 , 𝑩 : ( B , L , D , N ) ← discretize ( Δ , 𝑨 , 𝑩 ) 6: 𝑦 ← SSM ( 𝑨 , 𝑩 , 𝑪 )( 𝑥 ) ⊲ Time-varying : recurrence ( scan ) only 7: return 𝑦
...
![p8_img001](images/2312.00752v2-p8-img001.png)

### Table 2

[View CSV](tables/2312.00752v2-p8-table_2.csv)

### Table 3

[View CSV](tables/2312.00752v2-p8-table_3.csv)
...
Boundary Resetting. In settings where multiple independent sequences are stitched together, Transformers can keep them separate by instantiating a particular attention mask, while LTI models will bleed information between the sequences. Selective SSMs can also reset their state at boundaries (e.g. Δ 𝑡 →∞ , or Theorem 1 when 𝑔 𝑡 → 1). These settings may occur artificially (e.g. packing documents together to improve hardware utilization) or naturally (e.g. episode boundaries in reinforcement learning (Lu et al. 2023 )).
...
![p11_img001](images/2312.00752v2-p11-img001.png)
...
![p11_img002](images/2312.00752v2-p11-img002.png)
...
![p13_img001](images/2312.00752v2-p13-img001.png)
...
![p14_img001](images/2312.00752v2-p14-img001.png)
...
![p15_img001](images/2312.00752v2-p15-img001.png)
...
𝑨 𝑛 Initialization Field Perplexity

✗ ✗ ✗ 10.93 ✗ ✓ ✗ 10.15 ✗ ✗ ✓ 9.98 ✓ ✗ ✗ 9.81 ✓ ✓ ✓ 8.71

𝑨 𝑛 = − 1

2 + 𝑛𝑖 Complex 9.16 𝑨 𝑛 = − 1 / 2 Real 8.85 𝑨 𝑛 = −( 𝑛 + 1 ) Real 8.71 𝑨 𝑛 ∼ exp (N ( 0 , 1 )) Real 8.71
...
Δ 𝑡 = 𝜏 Δ ( Parameter + 𝑠 Δ ( 𝑥 𝑡 ))

= softplus ( Parameter + Linear ( 𝑥 𝑡 ))
...
𝑨 𝑡 = exp ( Δ 𝑨 ) = 1 1 + exp ( Linear ( 𝑥 𝑡 )) = 𝜎 (− Linear ( 𝑥 𝑡 ))

= 1 − 𝜎 ( Linear ( 𝑥 𝑡 ))

𝑩 𝑡 = ( Δ 𝑨 ) − 1 ( exp ( Δ 𝑨 ) − 𝑰 ) · Δ 𝑩 = −( exp ( Δ 𝑨 ) − 𝑰 ) = 1 − 𝑨
...
Params n _ layers d _ model n _ heads / d _ head Training steps Learning Rate Batch Size Tokens
...
![p31_img001](images/2312.00752v2-p31-img001.png)
...
• When the context length 𝐿 is less than (or equal to) 2 17 , we divide up each segment into non-overlapping sub-segments of length 𝐿 , so that there are 𝑆 × 2 17

𝐿 total samples and 𝑆 × 2 17 ≈ 4 . 5 𝐵 tokens per epoch.
...
Additional Ablations: SSM Parameterizations. We investigate SSM parameterizations on long-form audio waveform pretraining in the setting of Figure 7 . The setting is modified slightly to use larger models (8 layers and 𝐷 = 64 for 6M params, the SaShiMi default), shorter sequences (2 11 = 2048 to 2 18 = 262144 instead of 2 13 to 2 20 ), lower LR (0 . 001 from 0 . 002), and shorter training cycles (100K instead of 200K steps).
...
![p35_img001](images/2312.00752v2-p35-img001.png)
