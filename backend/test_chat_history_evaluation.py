#!/usr/bin/env python3
"""
Test file for evaluating chat history functionality with various question scenarios.
This script tests the ability of the chat system to:
1. Maintain conversation context across multiple questions
2. Reference previous questions and answers
3. Handle image-based questions with context
4. Provide coherent responses based on chat history
"""
...
class ChatHistoryEvaluator:
    ...
    async def populate_initial_chat_history(self):
        ...
        print("📝 Populating initial chat history...")
        ...
        for conv in initial_conversations:
            ...
            try:
                ...
                print(f"⚠️ Warning: Failed to embed message: {e}")
        
        print(f"✅ Added {len(initial_conversations)} messages to chat history")
    ...
    async def test_question_scenario(self, question_data: Dict[str, Any], test_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"🔍 Found {len(similar_qa_pairs)} similar Q&A pairs")
        except Exception as e:
            print(f"⚠️ Warning: Failed to search similar Q&A pairs: {e}")
            similar_qa_pairs = []
            ...
            for img_path in question_data['user_images']:
                try:
                    ...
                    if full_path.exists():
                        ...
                        print(f"🖼️ Processed image: {img_path}")
                    else:
        ...
        try:
            ...
            print(f"⚠️ Warning: Failed to embed messages: {e}")
    ...
    async def run_all_tests(self):
        ...
        print("🚀 Starting Chat History Evaluation Tests")
        print("=" * 60)
    ...
    async def generate_report(self):
        ...
        print("\n" + "=" * 60)
        print("📊 CHAT HISTORY EVALUATION REPORT")
        print("=" * 60)
        ...
        for result in self.results:
            ...
            if "error" in result:
                ...
                print(f"  📊 Context: {eval_data['context_awareness']:.2f}, Reference: {eval_data['reference_accuracy']:.2f}")
                print(f"  💬 Coherence: {eval_data['coherence']:.2f}, Completeness: {eval_data['completeness']:.2f}")
                print(f"  🔍 Chat History Used: {result['chat_history_used']} messages")
                print(f"  🎯 Similar Q&A Found: {result['similar_qa_found']}")
        ...
        report_filename = f"chat_history_evaluation_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
if __name__ == "__main__":
    print("🧪 Chat History Evaluation Test Suite")
    print("This script will test the chat history functionality with various scenarios.")
    print("Make sure the backend services are running and MongoDB is accessible.")
    print()
