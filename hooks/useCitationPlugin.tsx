...
type TabKey = string;
const DEFAULT_TAB_KEY: TabKey = "__default__";
...
interface CitationPluginProps {
    tabId?: string; // Optional tab identifier for state isolation
...
const getTabKey = (tabId?: string, pdfUrl?: string): TabKey => tabId || pdfUrl || DEFAULT_TAB_KEY;
...
export const useCitationPlugin = (props?: CitationPluginProps): Plugin => {
    ...
    const tabKey = getTabKey(tabId, pdfUrl);
    const {
        updateCitations,
        updateValidIds,
    ...
    const normalizedCitations = useMemo(() => extractedCitations ?? [], [extractedCitations]);

    useEffect(() => {
        const currentState = getTabState(tabKey);
        const currentCitations = currentState.extractedCitations ?? [];

        const citationsChanged =
            currentCitations.length !== normalizedCitations.length ||
            currentCitations.some((item: any, index: number) => !Object.is(item, normalizedCitations[index]));

        if (!citationsChanged) {
            return;
        }

        updateCitations(tabKey, normalizedCitations);
    }, [getTabState, tabKey, normalizedCitations, updateCitations]);

    console.log('[CitationPlugin]', `Initialized/Updated for tab "${tabKey}" with`, extractedCitations.length, 'citations');

    // Load and filter PDF.js annotations on plugin initialization
    const loadPDFAnnotations = useCallback(async (url: string, targetTabKey: TabKey) => {
        try {
            ...
            for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
                ...
                citationAnnotations.forEach((ann: any) => {
                    if (ann.id && ann.dest) {
                        ...
                        destinationMap.set(ann.id, ann.dest);
                        console.log(`✓ Valid citation found for tab "${targetTabKey}": ${ann.id} -> ${ann.dest}`);
                    }
                });
            }

            updateValidIds(targetTabKey, allValidIds);
            updateAnnotationMapping(targetTabKey, destinationMap);
            console.log(`📚 Loaded ${allValidIds.size} valid citation IDs for tab "${targetTabKey}"`);
            
        } catch (error) {
            console.error("Failed to load PDF annotations:", error);
        }
    }, [updateAnnotationMapping, updateValidIds]);
    ...
    useEffect(() => {
        ...
        loadPDFAnnotations(pdfUrl, tabKey).catch((err: unknown) => {
            console.error("Failed to initialize PDF annotations:", err);
        });
    }, [loadPDFAnnotations, pdfUrl, tabKey]);
    ...
    return {
        onDocumentLoad: async (e: any) => {
            ...
            if (!pdfUrl && e.doc) {
                try {
                    ...
                    if (pdfSource) {
                        await loadPDFAnnotations(pdfSource, tabKey);
                    }
                } catch (error) {
        ...
        onAnnotationLayerRender: (e: PluginRenderEvent) => {
            ...
            const currentTabState = getTabState(tabKey);
            const extractedForTab = currentTabState.extractedCitations ?? [];
            const validIdsForTab = currentTabState.validCitationIds ?? new Set<string>();
            const annotationDestinations = currentTabState.annotationIdToDestination ?? new Map<string, string>();

            console.log('[onAnnotationLayerRender]', `tab "${tabKey}" has`, extractedForTab.length, 'extracted citations');
            ...
            citationLinks.forEach((link: Element) => {
                ...
                if (!annotationId || !validIdsForTab.has(annotationId)) {
                    console.log(`❌ Skipping non-citation link: ${annotationId}`);
                    return;
                }

                console.log(`✅ Processing valid citation for tab "${tabKey}": ${annotationId}`);
                ...
                anchorLink.addEventListener("click", (ev: MouseEvent) => {
                    ...
                    const freshTabState = getTabState(tabKey);
                    const freshExtractedForTab = freshTabState.extractedCitations ?? [];
                    const freshAnnotationDestinations = freshTabState.annotationIdToDestination ?? new Map<string, string>();
                    console.log(`🔍 Searching for citation. AnnotationId: ${annotationId}, Available citations:`, freshExtractedForTab.length);

                    // Get the citation destination (cite.xxx) from the annotation ID
                    const citationDestination = freshAnnotationDestinations.get(annotationId);
                    console.log(`📍 Annotation ${annotationId} maps to destination:`, citationDestination);

                    // Match by destination
                    const extractedCitation = freshExtractedForTab.find((extracted: any) => {
                    ...
                    if (!extractedCitation) {
                        console.warn(`❌ No extracted citation found for ${annotationId} (dest: ${citationDestination}). Available IDs:`,
                            freshExtractedForTab.slice(0, 5).map((c: any) => c.id));
                    }
